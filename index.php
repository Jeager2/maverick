    ...
    <?php require 'menu.php';?>
      ...
      <div class="jumbotron row">
		   <div id="leftColumn" class="col-sm-9 col-xs-12">
			      <div id="roundwhite">
					  <div id="charttitle" class="col-xs-12">
						  <h2><span id="when"></span> <span id="live" class="label label-danger"><?=($activecookgauge['cookid'] < 0 ? '' : 'LIVE');?></span></h2>
					  </div>
		   ...
		   <div id="rightColumn" class="col-sm-3 col-xs-12">
			   <div class="btn-group" data-toggle="buttons">
			     <label class="btn btn-sm btn-default<?=($_SESSION['auth'] ? '' : ' active');?>">
			       <input type="radio" name="options" id="togText" autocomplete="off" <?=($_SESSION['auth'] ? '' : 'checked ')?>/> Text
			     </label>
			     <label class="btn btn-sm btn-default">
			       <input type="radio" name="options" id="togGauges" autocomplete="off" /> Gauges
			     </label>
				 <?php if ($_SESSION['auth']) : ?>
			     <label class="btn btn-sm btn-default active">
			       <input type="radio" name="options" id="togStart" autocomplete="off" checked /> Control
			     </label>
			 	<?php endif; ?>
			   </div>
	<!-- Display of TEXT tab begins here -->
				<div id="tempDiv" style="display:<?=($_SESSION['auth'] ? 'none' : 'block');?>">
	  				 <div><p>BBQ</p><span id="pit">&nbsp;&nbsp;&nbsp;&nbsp;</span></div>
	  				 <div><p>Food</p><span id="food">&nbsp;&nbsp;&nbsp;&nbsp;</span></div>
	  	 	 </div>
	<!-- Display of TEXT tab END here -->
	<!-- Display of GAUGES tab START here -->
				 <div id="gaugeDiv" style="display:none">
					 <div id="pit_div"></div>
					 <div id="food_div"></div>
				</div>
	<!-- Display of CONTROL tab START here -->
		...
		<form id="alertsForm" method="POST">
			<div id="alertsDiv" style="<?=$showAlertsRow?>">
		   	    ...
		   	    <label for="pitHigh" class="sr-only">Pit High</label>
				<div class="input-group">
			          <div class="input-group-addon mwfront">Pit <span class="fas fa-arrow-up text-danger"></span></div>
					  <input type="number" class="form-control" name="pitHi" id="pitHi" min="1" max="500" <?=($pH ? 'value="'.$pH.'" ' : '');?>placeholder="Pit High" />
					  <div class="input-group-addon">&deg;F</div>
				</div>
				<label for="pitLow" class="sr-only">Pit Low</label>
				<div class="input-group">
			          <div class="input-group-addon mwfront">Pit <span class="fas fa-arrow-down text-primary"></span></div>
		   	    	<input type="number" class="form-control" name="pitLow" id="pitLow" min="1" max="500" <?=($pL ? 'value="'.$pL.'" ' : '');?>placeholder="Pit Low" />
					<div class="input-group-addon">&deg;F</div>
				</div>
		   	    <label for="foodHigh" class="sr-only">Food High</label>
				<div class="input-group">
			          <div class="input-group-addon mwfront">Food <span class="fas fa-arrow-up text-danger"></span></div>
				...
				<div class="input-group">
			          <div class="input-group-addon mwfront">Food <span class="fas fa-arrow-down text-primary"></span></div>
